...
El desarrollo se realizó en colaboración con el laboratorio LaFHIS de la Facultad de Ciencias Exactas y Naturales de la Universidad de Buenos Aires.
...
The development was carried out in collaboration with the LaFHIS laboratory at the Faculty of Exact and Natural Sciences of the University of Buenos Aires.
...
\begin{itemize}
    ...
    \item \textbf{Compromiso de entradas}: las entradas se consideran confirmadas o \textit{committed} cuando han sido replicadas en una mayoría de nodos, asegurando su persistencia y consistencia a pesar de fallos.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Log Matching}: si dos logs contienen una entrada con el mismo índice y \textit{term}, entonces el historial de logs es idéntico hasta ese índice. Esto garantiza que las copias de log en distintos nodos mantengan una estructura coherente.
...
Las herramientas actuales para la validación de protocolos distribuidos se enfocan principalmente en la detección de fallas como \textit{crashes} o problemas de concurrencia. Sin embargo, no permiten verificar si la implementación realmente cumple con la lógica de negocio especificada. Es decir, aunque pueden identificar errores técnicos en la ejecución del sistema, no aseguran que el protocolo implementado respete las garantías y comportamientos esperados. Esto deja un vacío en la validación funcional de las implementaciones, lo que puede derivar en errores sutiles pero críticos en entornos distribuidos.

\bigskip

\section{Solución implementada}

Para abordar el problema de validación funcional, se propuso utilizar abstracciones preservadoras de habilitación (EPAs) que capturen el comportamiento observado de la implementación y permitan compararlo con las especificaciones formales del protocolo. Para generar estas EPAs automáticamente, se adopta la siguiente estrategia: instrumentar el código para exponer su estado interno en puntos relevantes, ejecutar el sistema bajo condiciones adversas controladas para explorar el espacio de estados, y procesar las trazas de ejecución para construir los grafos de abstracciones.

\textbf{Abstraktor} es una implementación de esta estrategia en forma de herramienta de línea de comandos que automatiza la generación de EPAs para protocolos distribuidos implementados en C/C++. La herramienta integra diferentes componentes y aplicaciones en un flujo de trabajo unificado. El código fuente está disponible públicamente en \url{https://github.com/matias-gonz/abstraktor}.
...
\draw[arrow] (cli) -- (llvm) node[midway, above left, edgelabel] {JSON targets};
\draw[arrow] (cli) -- (mallory) node[midway, above right, edgelabel] {logs eventos};
...
\begin{itemize}
    \item \textbf{clap}: framework para parseo de argumentos de línea de comandos con soporte para subcomandos.
    \item \textbf{serde} y \textbf{serde\_json}: serialización y deserialización de estructuras de datos.
    \item \textbf{regex}: análisis de patrones en código fuente para extracción de anotaciones.
    \item \textbf{xshell}: ejecución de comandos del sistema con manejo de directorios y variables de entorno.
    \item \textbf{anyhow}: manejo de errores con contexto enriquecido.
\end{itemize}

La elección de Rust como lenguaje para el CLI de Abstraktor se basó en la experiencia previa con el lenguaje y sus bibliotecas, además de ser preferencia personal del equipo de desarrollo.
...
\draw[call] (3.5,-1.5) -- (4.5,-1.5) -- (4.5,-2) -- (3.5,-2);
\node[note] at (5.2,-1.75) {get-targets};
\draw[call] (3.5,-2.8) -- (7,-2.8) node[midway, above, note] {instrumentar\\(JSON targets)};
\draw[return] (7,-3.3) -- (3.5,-3.3) node[midway, above, note] {binario};
\draw[return] (3.5,-3.8) -- (0,-3.8) node[midway, above, note] {binario};

\draw[call] (0,-4.3) -- (3.5,-4.3) node[midway, above, note] {run mallory};
\draw[call] (3.5,-4.8) -- (10.5,-4.8) node[midway, above, note] {ejecutar};
\draw[return] (10.5,-5.3) -- (3.5,-5.3) node[midway, above, note] {logs eventos};
\draw[return] (3.5,-5.8) -- (0,-5.8) node[midway, above, note] {logs eventos};

\draw[call] (0,-6.3) -- (3.5,-6.3) node[midway, above, note] {export-graphs};
\draw[return] (3.5,-6.8) -- (0,-6.8) node[midway, above, note] {EPAs};
...
El desarrollo de \textbf{Abstraktor} se llevó a cabo aplicando prácticas de ingeniería de software modernas, con un enfoque ágil basado en iteraciones semanales. Esta sección describe el contexto en el que se realizó el desarrollo, el proceso aplicado, las herramientas utilizadas y las prácticas de calidad implementadas.
...
El equipo de desarrollo estuvo compuesto por dos desarrolladores: Tomás Chimenti, un estudiante de ciencias de la computación trabajando en su tesis y Matías González, un estudiante de ingeniería informática completando su Trabajo Profesional Final. Ambos estudiantes trabajaron guiados por Sebastián Uchitel, chair de LaFHIS, que cumplió el rol de Product Owner. Adicionalmente, Nicolás Paez, director del Trabajo Profesional Final, actuó como facilitador de reuniones y supervisor del desarrollo, asegurando la dirección y calidad del proyecto.

La experiencia de desarrollo colaborativo entre un estudiante de ingeniería y uno de ciencias de la computación, con sus desafíos relacionados con la divergencia de objetivos y el trabajo con código legacy, fue documentada en un artículo aceptado en el 1st International Workshop on Software Engineering and Research Software (SERS 2026)\footnote{\url{https://conf.researchr.org/home/icse-2026/sers-2026}}, co-ubicado con ICSE 2026. El artículo reflexiona sobre las estrategias aplicadas para balancear experimentación rápida con desarrollo de software mantenible.
...
Se utilizó \textbf{GitHub Projects} para organizar el trabajo mediante un tablero Kanban, donde las tareas se priorizaron según su impacto en el proyecto y sus dependencias con otros componentes. Cada tarea fue estimada utilizando puntajes de dificultad (1, 2 o 3).
...
\begin{itemize}
    \item \textbf{Tests automatizados}: se implementaron \textbf{73 tests}.
    
    \item \textbf{Cobertura de código}: se alcanzó una cobertura general del 86\% medida con Codecov. El módulo central del modelo alcanzó una cobertura del 99.51\%, donde las secciones no cubiertas fueron validadas mediante tests de integración manuales.
    
    \item \textbf{Pull Requests}: se realizaron 51 pull requests durante el desarrollo.
    
    \item \textbf{Issues}: se gestionaron 76 issues en GitHub Projects.
\end{itemize}
...
El impacto social de \textbf{Abstraktor} se manifiesta principalmente en su contribución a la confiabilidad de sistemas distribuidos. Estos sistemas son fundamentales para servicios críticos como bases de datos, sistemas de almacenamiento y plataformas de comunicación. Al facilitar la validación de protocolos distribuidos, la herramienta contribuye a mejorar la robustez de mucho software del que depende la sociedad.

Adicionalmente, el proyecto se desarrolló como software de código abierto, promoviendo la colaboración académica y facilitando que otros investigadores puedan utilizar y extender la herramienta.
...
Los principales desafíos enfrentados estuvieron relacionados con la incertidumbre técnica en las soluciones de implementación y la integración con software legacy. El trabajo con Mallory, un sistema con poca documentación y tests, evidenció la importancia crítica de estas prácticas para la mantenibilidad del software. La falta de documentación incrementó significativamente el tiempo necesario para comprender el sistema antes de poder extenderlo, mientras que la ausencia de tests dificultó evaluar el impacto de los cambios realizados.

Más allá de los desafíos técnicos, surgieron dificultades relacionadas con la colaboración del equipo. Tomás, como estudiante de ciencias de la computación, priorizaba la experimentación rápida para su investigación, mientras que Matías, como estudiante de ingeniería, ponía énfasis en construir un sistema robusto y mantenible. Esta divergencia de objetivos ocasionalmente dificultó el desarrollo.

Para abordar estos desafíos, se aplicaron las siguientes estrategias: se estableció un enfoque híbrido que permitía experimentación rápida en branches separadas mientras se mantenían estándares de calidad al integrar código al repositorio principal. Adicionalmente, el desarrollo de tests de integración proporcionó mayor confianza al modificar el código legacy. La experiencia refuerza la necesidad de priorizar la documentación y el testing desde las etapas tempranas del desarrollo, especialmente en proyectos que eventualmente serán extendidos o mantenidos por otros desarrolladores.
