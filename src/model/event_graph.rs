...
fn parse_mediator_log_line(line: &str) -> Option<EventRecord> {
    let relevant_part = line.find("[Node ")?;
    let relevant_part = &line[relevant_part..];

    let node_start = "[Node ".len();
